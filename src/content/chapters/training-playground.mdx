...
4. **Push the rate — and notice what *doesn't* happen.** Set the learning
   ...
   (−1, 1), so this loss surface has no cliff to fall off — and on these
   small, full-batch problems the hot rate actually *wins* (try Spiral at
   1.0 versus 0.1; the fast run reaches a loss the slow one won't touch).
   Don't over-learn that lesson, though: real training runs use noisier
   gradients and unbounded losses, where chapter 3's 💥 sits one hot step
   away. The seatbelt here is the architecture, not a law of nature.
