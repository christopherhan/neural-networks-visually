...
this book's plain word for what the math calls a gradient and writes as
∂loss/∂value: the change in the loss per tiny change in this one value,
everything else held still. The curly ∂ is chapter 3's slope idea, asked
of one knob among millions.
...
<CodeTabs variants={[
  { label: 'TypeScript', lang: 'ts', code: `// value.ts — the heart of it. Each op records how to route blame:
mul(other) {
...
backward() {
  ...
  { label: 'Python', lang: 'python', code: `# value.py — the heart of it. Each op records how to route blame:
...
def backward(self):
    ...
    for node in reverse_topo_order(self):
        node._backward()` },
  { label: 'Go', lang: 'go', code: `// value.go — the heart of it. Each op records how to route blame:
...
func (v *Value) Backward() {
  ...
  { label: 'Java', lang: 'java', code: `// Value.java — the heart of it. Each op records how to route blame:
...
void backward() {
    // topologically sort the graph, then apply the chain rule in reverse
    this.grad = 1;
    for (Value node : reverseTopoOrder(this)) node.backward.run();
}` },
]} />

The TypeScript tab is not pseudocode; it's lightly trimmed from
[`src/lib/nn/value.ts`](https://github.com/christopherhan/neural-networks-visually/blob/main/src/lib/nn/value.ts)
in this site's repository, the code that actually ran the figure above. The
other tabs are line-for-line ports so you can read it in whichever language
you think in. PyTorch and JAX are this exact idea, plus tensors, plus a
